...
def test_products_loaded_on_listing(driver: WebDriver):
    """Pagination - Page inexistante - positif : produits charges sur la page listing."""
...
def test_next_page_or_single_page(driver: WebDriver):
    """Pagination - Page inexistante - positif : bouton Next si plusieurs pages, sinon page unique OK."""
    _go_to_listing(driver, BASE_URL)
    WebDriverWait(driver, 20).until(
        EC.presence_of_element_located((By.CSS_SELECTOR, ".product-item"))
    )
...
def test_invalid_page_url(driver: WebDriver):
    """Pagination - Page inexistante - negatif : URL de page invalide."""
