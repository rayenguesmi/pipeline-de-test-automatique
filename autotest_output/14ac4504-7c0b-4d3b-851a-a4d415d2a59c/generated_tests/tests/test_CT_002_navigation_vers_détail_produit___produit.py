...
def test_navigate_to_product_detail(driver: WebDriver):
    """Navigation vers détail produit - Produit indisponible - positif : clic produit ouvre la page detail."""
...
def test_product_detail_has_price(driver: WebDriver):
    """Navigation vers détail produit - Produit indisponible - positif : page detail affiche un prix."""
    _go_to_listing(driver, BASE_URL)
    link = WebDriverWait(driver, 20).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a"))
    )
...
def test_detail_page_has_product_name(driver: WebDriver):
    """Navigation vers détail produit - Produit indisponible - positif : page detail affiche le nom du produit."""
    _go_to_listing(driver, BASE_URL)
    link = WebDriverWait(driver, 20).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "a"))
    )
