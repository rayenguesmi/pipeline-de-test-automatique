...
def test_search_returns_results(driver: WebDriver):
    """Recherche avec des caractères spéciaux - positif : recherche 'camera' retourne des resultats."""
    driver.get(BASE_URL)
...
def test_search_no_results(driver: WebDriver):
    """Recherche avec des caractères spéciaux - negatif : recherche inexistante -> message no results."""
    driver.get(BASE_URL)
    search = WebDriverWait(driver, 20).until(
        EC.presence_of_element_located((By.CSS_SELECTOR, "#small-searchterms"))
    )
...
def test_search_with_partial_keyword(driver: WebDriver):
    """Recherche avec des caractères spéciaux - positif : recherche partielle 'phone'."""
    driver.get(BASE_URL)
    search = WebDriverWait(driver, 20).until(
        EC.presence_of_element_located((By.CSS_SELECTOR, "#small-searchterms"))
    )
